...
class NXCHTTPServer(threading.Thread):
    def __init__(self, module, context, logger, srv_host, port, server_type="https"):
        try:
            ...
            self.server.log = NXCAdapter(extra={"module_name": self.server.module.name.upper()})
            self.cert_path = os.path.join(os.path.expanduser("~/.nxc"), "nxc.pem")
            self.server.track_host = self.track_host

            logger.debug("nxc server type: " + server_type)
