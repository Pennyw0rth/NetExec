...
class ssh(connection):
    ...
    def load_private_key(self, key_content):
        key_types = [paramiko.RSAKey, paramiko.DSSKey, paramiko.ECDSAKey, paramiko.Ed25519Key]

        for key_type in key_types:
            try:
                pkey = key_type.from_private_key(StringIO(key_content))
                self.logger.debug(f"Successfully loaded a {key_type.__name__} with key file: {self.args.key_file}")
                return pkey
    ...
    def connect_with_key(self, username, password, key_content):
        ...
        pkey = self.load_private_key(key_content)
        if pkey:
            self.conn.connect(self.host, port=self.port, username=username, passphrase=password, pkey=pkey, look_for_keys=False, allow_agent=False, disabled_algorithms={"pubkeys": ["rsa-sha2-256", "rsa-sha2-512"]})
        else:
            self.logger.debug(f"Could not load key file: {self.args.key_file}")
