...
from nxc.protocols.smb.kerberos import (
    kerberos_login_with_S4U,
    kerberos_altservice,
    get_realm_from_ticket,
)
...
class smb(connection):
    ...
    def enum_host_info(self):
        ...
        if not self.no_ntlm:
            ...
            if (
                not self.targetDomain
            ):  # Not sure if that can even happen but now we are safe
        ...
        if self.args.domain:
            ...
            self.args.use_kcache
        ):  # Fixing domain trust, just pull the auth domain out of the ticket
        ...
        if (
            "Windows 6.1" in self.server_os
            and self.server_os_build == 0
            and self.os_arch == 0
        ):
            ...
            self.server_os = "Unix"
        self.logger.debug(
            f"Server OS: {self.server_os} {self.server_os_major}.{
                self.server_os_minor
            } build {self.server_os_build}"
        )
        ...
        self.remoteName = (
            self.host if not self.kerberos else f"{self.hostname}.{self.targetDomain}"
        )
    ...
    def print_host_info(self):
        signing = (
            colored(f"signing:{self.signing}", host_info_colors[0], attrs=["bold"])
            if self.signing
            else colored(f"signing:{self.signing}", host_info_colors[1], attrs=["bold"])
        )
        smbv1 = (
            colored(f"SMBv1:{self.smbv1}", host_info_colors[2], attrs=["bold"])
            if self.smbv1
            else colored(f"SMBv1:{self.smbv1}", host_info_colors[3], attrs=["bold"])
        )
        ntlm = (
            colored(f" (NTLM:{not self.no_ntlm})", host_info_colors[2], attrs=["bold"])
            if self.no_ntlm
            else ""
        )
        null_auth = (
            colored(
                f" (Null Auth:{self.null_auth})", host_info_colors[2], attrs=["bold"]
            )
            if self.null_auth
            else ""
        )
        guest = (
            colored(
                f" (Guest Auth:{self.is_guest})", host_info_colors[1], attrs=["bold"]
            )
            if self.is_guest
            else ""
        )
        self.logger.display(
            f"{self.server_os}{f' x{self.os_arch}' if self.os_arch else ''} (name:{
                self.hostname
            }) (domain:{self.targetDomain}) ({signing}) ({smbv1}){ntlm}{null_auth}{
                guest
            }"
        )
        ...
        if self.args.generate_hosts_file or self.args.generate_krb5_file:
            ...
            if self.args.generate_hosts_file:
                with open(self.args.generate_hosts_file, "a+") as host_file:
                    ...
                    host_file.write(
                        f"{self.host}     {self.hostname}.{self.targetDomain}{dc_part} {
                            self.hostname
                        }\n"
                    )
                    self.logger.debug(
                        f"Line added to {self.args.generate_hosts_file} {self.host}    {
                            self.hostname
                        }.{self.targetDomain}{dc_part} {self.hostname}"
                    )
                ...
                with open(self.args.generate_krb5_file, "w+") as host_file:
                    ...
                    self.logger.success(
                        f"Run the following command to use the conf file: export KRB5_CONFIG={
                            self.args.generate_krb5_file
                        }"
                    )
        ...
        try:
            ...
            if self.args.delegate:
                ...
                serverName = Principal(
                    self.args.delegate_spn
                    if self.args.delegate_spn
                    else f"cifs/{self.remoteName}",
                    type=constants.PrincipalNameType.NT_SRV_INST.value,
                )
                tgs, sk = kerberos_login_with_S4U(
                    domain,
                    self.hostname,
                    username,
                    password,
            ...
            if self.args.delegate_spn:
                used_ccache = (
                    f" through S4U with {username} (w/ SPN {self.args.delegate_spn})"
                )
            ...
            if self.args.delegate:
                used_ccache = f" through S4U with {username}"
            self.logger.fail(
                f"{domain}\\{self.username}{used_ccache} {error} {
    ...
    def plaintext_login(self, domain, username, password):
        ...
        try:
            ...
            if self.admin_privs and valid_auth:
                self.logger.debug(
                    f"Adding admin user: {self.domain}/{self.username}:{self.password}@{
                        self.host
                    }"
                )
            ...
            self.logger.success(
                f"{domain}\\{self.username}:{process_secret(self.password)} {
                    self.mark_guest()
                }{self.mark_pwned()}"
            )
            ...
            error, desc = e.getErrorString()
            self.logger.fail(
                f"{domain}\\{self.username}:{process_secret(self.password)} {error} {
                    f'({desc})' if self.args.verbose else ''
                }",
                color="magenta" if error in smb_error_status else "red",
            )
            if error in [
                ...
                "STATUS_PASSWORD_EXPIRED",
                "STATUS_NOLOGON_WORKSTATION_TRUST_ACCOUNT",
            ] and self.args.module == ["change-password"]:
                return True
            if error not in smb_error_status:
                self.inc_failed_login(username)
                return False
            ...
            desc = e.getErrorString() if hasattr(e, "getErrorString") else str(e)
            self.logger.fail(
                f"{domain}\\{self.username}:{process_secret(self.password)} {desc}"
            )
            return False
        except BrokenPipeError:
            self.logger.fail("Broken Pipe Error while attempting to login")
    ...
    def hash_login(self, domain, username, ntlm_hash):
        ...
        try:
            ...
            self.logger.success(
                f"{domain}\\{self.username}:{process_secret(self.hash)} {
                    self.mark_guest()
                }{self.mark_pwned()}"
            )
            ...
            error, desc = e.getErrorString()
            self.logger.fail(
                f"{domain}\\{self.username}:{process_secret(self.hash)} {error} {
                    f'({desc})' if self.args.verbose else ''
                }",
                color="magenta" if error in smb_error_status else "red",
            )
    ...
    def execute(self, payload=None, get_output=False, methods=None) -> str:
        ...
        if "exec_method" in locals():
            ...
            output = "\n".join([
                ll.rstrip() for ll in output.splitlines() if ll.strip()
            ])
    ...
    def ps_execute(
        ...
        """
        payload = (
            self.args.ps_execute if not payload and self.args.ps_execute else payload
        )
        if not payload:
        ...
        self.logger.debug(
            f"Starting ps_execute(): {payload=} {get_output=} {
                          methods=} {force_ps32=} {obfs=} {encode=}"
        )
    ...
    def get_session_list(self):
        with TSTS.TermSrvEnumeration(self.conn, self.host, self.kerberos) as lsm:
            ...
            for i in rsessions:
                ...
                state = TSTS.enum2value(TSTS.WINSTATIONSTATECLASS, sess["State"]).split(
                    "_"
                )[-1]
    ...
    def enumerate_sessions_info(self, sessions):
        if len(sessions):
            with TSTS.TermSrvSession(
                self.conn, self.host, self.kerberos
            ) as TermSrvSession:
                for SessionId in sessions:
                    sessdata = TermSrvSession.hRpcGetSessionInformationEx(SessionId)
                    sessflags = TSTS.enum2value(
                        TSTS.SESSIONFLAGS,
                        sessdata["LSMSessionInfoExPtr"]["LSM_SessionInfo_Level1"][
                            "SessionFlags"
                        ],
                    )
                    sessions[SessionId]["flags"] = sessflags
                    domain = sessdata["LSMSessionInfoExPtr"]["LSM_SessionInfo_Level1"][
                        "DomainName"
                    ]
                    if not len(sessions[SessionId]["Domain"]) and len(domain):
                        sessions[SessionId]["Domain"] = domain
                    username = sessdata["LSMSessionInfoExPtr"][
                        "LSM_SessionInfo_Level1"
                    ]["UserName"]
                    if not len(sessions[SessionId]["Username"]) and len(username):
                        sessions[SessionId]["Username"] = username
                    sessions[SessionId]["ConnectTime"] = sessdata[
                        "LSMSessionInfoExPtr"
                    ]["LSM_SessionInfo_Level1"]["ConnectTime"]
                    sessions[SessionId]["DisconnectTime"] = sessdata[
                        "LSMSessionInfoExPtr"
                    ]["LSM_SessionInfo_Level1"]["DisconnectTime"]
                    sessions[SessionId]["LogonTime"] = sessdata["LSMSessionInfoExPtr"][
                        "LSM_SessionInfo_Level1"
                    ]["LogonTime"]
                    sessions[SessionId]["LastInputTime"] = sessdata[
                        "LSMSessionInfoExPtr"
                    ]["LSM_SessionInfo_Level1"]["LastInputTime"]
                ...
                with TSTS.RCMPublic(self.conn, self.host, self.kerberos) as rcm:
                    for SessionId in sessions:
                        try:
                            ...
                            if not client:
                                continue
                            sessions[SessionId]["RemoteIp"] = client["pRemoteAddress"][
                                "ipv4"
                            ]["in_addr"]
                        except Exception as e:
                            self.logger.debug(
        ...
        with TSTS.LegacyAPI(self.conn, self.host, self.kerberos) as legacy:
            ...
            if self.args.taskkill.isdigit():
                ...
                pidList = [
                    i["UniqueProcessId"]
                    for i in res
                    if i["ImageName"].lower() == self.args.taskkill.lower()
                ]
                if not pidList:
                    self.logger.fail(
    ...
    def qwinsta(self):
        ...
        maxUsernameLen = (
            max(
                len(sessions[i]["Username"] + sessions[i]["Domain"]) + 1
                for i in sessions
            )
            + 1
        )
        ...
        template = (
            f"{{SESSIONNAME: <{maxSessionNameLen}}} "
        ...
        for i in sessions:
            ...
            connectTime = (
                connectTime.strftime(r"%Y/%m/%d %H:%M:%S")
                if connectTime.year > 1601
                else "None"
            )

            disconnectTime = sessions[i]["DisconnectTime"]
            disconnectTime = (
                disconnectTime.strftime(r"%Y/%m/%d %H:%M:%S")
                if disconnectTime.year > 1601
                else "None"
            )
    ...
    def tasklist(self):
        ...
        try:
            with TSTS.LegacyAPI(self.conn, self.host, self.kerberos) as legacy:
                ...
                template = f"{{: <{maxImageNameLen}}} {{: <8}} {{: <11}} {{: <{
                    maxSidLen
                }}} {{: >12}}"
                self.logger.highlight(
                    template.format("Image Name", "PID", "Session#", "SID", "Mem Usage")
                )
                self.logger.highlight(
                    template.replace(": ", ":=").format("", "", "", "", "")
                )
                found_task = False
    ...
    def reg_sessions(self):
        ...
        def output(sessions):
            if sessions:
                ...
                maxUsernameLen = (
                    max(
                        len(str(vals["Username"]) + str(vals["Domain"])) + 1
                ...
                self.logger.info(
                    f"No active session found for specified user(s) using the Remote Registry service on {
                        self.hostname
                    }."
                )
        ...
        for binding_attempts in range(2, 0, -1):
            ...
            try:
                ...
                if binding_attempts == 1:  # Last attempt
                    self.logger.info(
                        f"The Remote Registry service seems to be disabled on {
                            self.hostname
                        }."
                    )
                    return
        ...
        try:
            ...
            if "rpc_s_access_denied" in str(e).lower():
                self.logger.info(
                    f"Access denied while enumerating session using the Remote Registry on {
                        self.hostname
                    }."
                )
                return
        ...
        try:
            ...
            self.logger.debug(
                f"Failed to connect to LSARPC for SID resolution on {self.hostname}: {
                    e
                }"
            )
            output(sessions)
        ...
        if resp:
            for sid, item in zip(
                sessions.keys(), resp["TranslatedNames"]["Names"], strict=False
            ):
                if item["DomainIndex"] >= 0:
                    sessions[sid]["Username"] = item["Name"]
                    sessions[sid]["Domain"] = resp["ReferencedDomains"]["Domains"][
                        item["DomainIndex"]
                    ]["Name"]
    ...
    def _check_subdir_write(self, share_name, path, depth, temp_dir_name):
        ...
        for entry in entries:
            ...
            try:
                ...
                self.logger.debug(
                    f"WRITE access confirmed in subdirectory: {subdir_path}"
                )
                continue
            except SessionError:
                pass
            ...
            if depth > 1:
                deeper = self._check_subdir_write(
                    share_name, subdir_path, depth - 1, temp_dir_name
                )
    ...
    def shares(self):
        ...
        for share in shares:
            ...
            try:
                ...
                error = get_error_string(e)
                self.logger.debug(
                    f"Error checking READ access on share {share_name}: {
                        error
                    }. This exception always caused by special character in share name with SMBv1"
                )
                self.logger.info(
                ...
                try:
                    ...
                    try:
                        ...
                        if error == "STATUS_OBJECT_NAME_NOT_FOUND":
                            ...
                            self.logger.debug(
                                f"Error DELETING created temp dir {temp_dir} on share {
                                    share_name
                                }: {error}"
                            )
                except SessionError as e:
                    error = get_error_string(e)
                    self.logger.debug(
                        f"Error checking WRITE access with DIR creation on share {
                            share_name
                        }: {error}"
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        if error == "STATUS_OBJECT_NAME_NOT_FOUND":
                            ...
                            self.logger.debug(
                                f"Error DELETING created temp file {
                                    temp_file
                                } on share {share_name}"
                            )
                except SessionError as e:
                    error = get_error_string(e)
                    self.logger.debug(
                        f"Error checking WRITE access with FILE creation on share {
                            share_name
                        }: {error}"
                    )
            ...
            if not write and read and write_check and self.args.shares_depth > 0:
                writable_subdirs = self._check_subdir_write(
                    share_name, "", self.args.shares_depth, gen_random_string()
                )
                if writable_subdirs:
                    write = True
    ...
    def dir(self):
        ...
        for content in contents:
            ...
            self.logger.highlight(
                f"{'d' if content.is_directory() else 'f'}{
                    'rw-' if content.is_readonly() > 0 else 'r--':<8}{
                    content.get_filesize():<15}{
                    ctime(float(content.get_mtime_epoch())):<30}{full_path:<45}"
            )
                ...
                while offset < len(response) and offset + 152 <= len(response):
                    try:
                        ...
                        next_offset = struct.unpack(
                            "<L", response[offset : offset + 4]
                        )[0]
                        if_index = struct.unpack(
                            "<L", response[offset + 4 : offset + 8]
                        )[0]
                        capabilities = struct.unpack(
                            "<L", response[offset + 8 : offset + 12]
                        )[0]
                        link_speed = struct.unpack(
                            "<Q", response[offset + 16 : offset + 24]
                        )[0]

                        # Socket address (SockAddr_Storage at offset+24)
                        family = struct.unpack(
                            "<H", response[offset + 24 : offset + 26]
                        )[0]
    ...
    def local_groups(self):
        ...
        if groups and not self.args.local_groups:
            ...
            for group_name, group_rid in groups.items():
                ...
                group_id = self.db.add_group(self.hostname, group_name, rid=group_rid)[
                    0
                ]
    ...
    def users(self):
        ...
        return UserSamrDump(self).dump(
            requested_users=self.args.users, dump_path=self.args.users_export
        )
    ...
    def loggedon_users(self):
        ...
        try:
            ...
            for user in response["UserInfo"]["WkstaUserInfo"]["Level1"]["Buffer"]:
                user_info = (
                    user["wkui1_logon_domain"][:-1],
                    user["wkui1_username"][:-1],
                    user["wkui1_logon_server"][:-1],
                )
    ...
    def wmi_query(self, wql=None, namespace=None, callback_func=None):
        ...
        try:
            ...
            if not flag or not stringBinding:
                error_msg = f"WMI Query: Dcom initialization failed on connection with stringbinding: '{
                    stringBinding
                }', please increase the timeout with the option '--dcom-timeout'. If it's still failing maybe something is blocking the RPC connection, try another exec method"
                ...
                self.logger.fail(error_msg) if not flag else self.logger.debug(
                    error_msg
                )
                ...
                if not callback_func:
                    while True:
                        ...
                        for k, v in record.items():
                            if (
                                k != "TimeGenerated"
                            ):  # from the wcc module, but this is a small hack to get it to stop spamming - TODO: add in method to disable output for this function
    ...
    def sccm(self):
        ...
        self.logger.success(
            ...
            } decrypted masterkeys. Looting SCCM Credentials through {self.args.sccm}"
        )
        ...
        def sccm_callback(secret):
            if isinstance(secret, SCCMCred):
                tag = "NAA Account"
                self.logger.highlight(
                    f"[{tag}] {secret.username.decode('latin-1')}:{
                ...
                self.logger.highlight(
                    f"[{tag}] {secret.variable.decode('latin-1')}:{
        ...
        try:
            sccm_triage = SCCMTriage(
                ...
                per_secret_callback=sccm_callback,
            )
    ...
    def dpapi(self):
        ...
        self.output_file = open(
            self.output_file_template.format(output_folder="dpapi"),
            "w",
            encoding="utf-8",
        )  # noqa: SIM115
        ...
        def credential_callback(credential):
            tag = "CREDENTIAL"
            line = f"[{credential.winuser}][{tag}] {credential.target} - {
                credential.username
            }:{credential.password}"
            self.logger.highlight(line)
            if self.output_file:
        ...
        try:
            ...
            for cng_file in cng_triage.triage_system_cng():
                if (
                    cng_file.cng_blob["Name"].decode("utf-16le").rstrip("\0")
                    == "Google Chromekey1"
                ):
        ...
        def browser_callback(secret):
            if isinstance(secret, LoginData):
                ...
                line = f"[{secret.winuser}][{secret.browser.upper()}] {secret_url} {
                    secret.username
                }:{secret.password}"
                self.logger.highlight(line)
                if self.output_file:
                ...
                line = f"[{secret.winuser}][{
                    secret.browser.upper()
                }] Google Refresh Token: {secret.service}:{secret.token}"
                self.logger.highlight(line)
                if self.output_file:
                    self.output_file.write(line + "\n")
                self.db.add_dpapi_secrets(
                ...
                line = f"[{secret.winuser}][{secret.browser.upper()}] {secret.host}{
                    secret.path
                } - {secret.cookie_name}:{secret.cookie_value}"
                self.logger.highlight(line)
                if self.output_file:
                    self.output_file.write(line + "\n")
        ...
        try:
            browser_triage = BrowserTriage(
                ...
                conn=conn,
                masterkeys=masterkeys,
                per_secret_callback=browser_callback,
            )
            browser_triage.triage_browsers(
                gather_cookies=dump_cookies, cng_chromekey=cng_chromekey
            )
        except Exception as e:
        ...
        def vault_callback(secret):
            ...
            if secret.type == "Internet Explorer":
                ...
                line = f"[{secret.winuser}][{tag}] {resource} - {secret.username}:{
                    secret.password
                }"
                self.logger.highlight(line)
                if self.output_file:
        ...
        try:
            ...
            vaults_triage = VaultsTriage(
                ...
                conn=conn,
                masterkeys=masterkeys,
                per_vault_callback=vault_callback,
            )
        ...
        def firefox_callback(secret):
            ...
            if isinstance(secret, FirefoxData):
                ...
                line = f"[{secret.winuser}][{tag}] {url} {secret.username}:{
                    secret.password
                }"
                self.logger.highlight(line)
                if self.output_file:
                ...
                line = f"[{secret.winuser}][{tag}] {secret.host}{secret.path} {
                    secret.cookie_name
                }:{secret.cookie_value}"
                self.logger.highlight(line)
                if self.output_file:
        ...
        try:
            ...
            firefox_triage = FirefoxTriage(
                ...
                logger=self.logger,
                conn=conn,
                per_secret_callback=firefox_callback,
            )
    ...
    def lsa(self):
        try:
            ...
            if self.remote_ops and self.bootkey:
                ...
                self.logger.success(
                    f"Dumped {highlight(add_lsa_secret.secrets)} LSA secrets to {
                        self.output_filename + '.secrets'
    ...
    def ntds(self):
        ...
        def add_hash(secret_type, secret, host_id):
            ...
            if (
                self.args.kerberos_keys
                and not printed_kerb_keys_banner
                and secret_type == NTDSHashes.SECRET_TYPE.NTDS_KERBEROS
            ):
            ...
            if (
                secret.find("$") == -1
                and secret_type == NTDSHashes.SECRET_TYPE.NTDS
                and "_history" not in secret
            ):
                if secret.find("\\") != -1:
        ...
        try:
            ...
            self.logger.success(
                f"Dumped {highlight(add_hash.nt_lm_secrets)} NTDS hashes to {
                    ntds_outfile
                } of which {highlight(add_hash.added_to_db)} were added to the database"
            )
            if self.args.kerberos_keys:
                self.logger.success(
                    f"Dumped {highlight(add_hash.kerb_secrets)} Kerberos keys to {
                        ntds_outfile
                    }.kerberos"
                )
            self.logger.display(
