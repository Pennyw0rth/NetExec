...
def proto_args(parser, parents):
    ...
    egroup.add_argument("--asreproasting", "--asreproast", help="Output AS_REP response to crack with hashcat to file")
    kerberoasting_arg = egroup.add_argument("--kerberoasting", "--kerberoast", help="Output TGS ticket to crack with hashcat to file")
    kerberoast_users_arg = egroup.add_argument("--kerberoasting-account", "--kerberoast-account", nargs="+", dest="kerberoast_account", action=get_conditional_action(_StoreAction), make_required=[], help="Target specific accounts for kerberoasting (sAMAccountNames or file containing sAMAccountNames)")
