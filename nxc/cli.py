...
""",
    ...
    # --- Attach module listing to the protocol help (epilog), do NOT create positional module subparsers ---
    for proto_name, proto_parser in list(proto_subparsers.choices.items()):
        ...
        # Keep existing description and append module list in epilog for clear display
        try:
            existing = proto_parser.description or ""
            proto_parser.description = existing
            proto_parser.epilog = (proto_parser.epilog or "") + "\n".join(lines)
        except Exception:
            # If proto_parser doesn't accept assignment, skip silently
            pass
