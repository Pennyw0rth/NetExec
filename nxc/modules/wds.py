...
class NXCModule:
    ...
    name = "wds"
    description = "Anonymously uses RPC endpoints to hunt for Windows Deployment Service (WDS) which are potentially hosting PXE images"
    ...
    def on_login(self, context, connection):
        ...
        for entry in entries:
            ...
            if tmpUUID[:36] in epm.KNOWN_PROTOCOLS:
                protocol = epm.KNOWN_PROTOCOLS[tmpUUID[:36]]
                context.log.debug("Protocol %s" % protocol)
                if "MS-WDSC" in protocol:
                    context.log.highlight("Windows Deployment Service Found.")
                    # Check if TFTP is running on WDS
                    tftp_port = 69
                    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                    sock.settimeout(5)
                    try:
                        sock.sendto(b'\x00\x01test\x00octet\x00', (connection.host, 69))  # TFTP Read Request format
                        data, _ = sock.recvfrom(1024)
                        if data:
                            context.log.highlight("TFTP service is running on UDP port 69.")
                    except Exception as e:
                        context.log.debug(f"Failed to contact TFTP service: {str(e)}")
                    finally:
                        sock.close()
                    return
