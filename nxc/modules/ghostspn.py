...
class NXCModule:
    name = "ghostspn"
    description = "Attempt to check if the OS is vulnerable to CVE-2025-58726 (GhostSPN attack)"
        ...
        try:
            ...
            if vuln:
                if not connection.conn.isSigningRequired():  # Not vulnerable if SMB signing is enabled
                    context.log.highlight(f"VULNERABLE (can relay SMB using Ghost SPN for Kerberos reflection on {self.context.log.extra['host']})")
                else:
                    context.log.highlight(f"VULNERABLE (can relay SMB using Ghost SPN (non HOST/CIFS) for Kerberos reflection to other protocols except SMB on {self.context.log.extra['host']})")
            ...
            context.log.debug(f"ghostspn: DCERPC transport error: {e.__class__.__name__}: {e}")
