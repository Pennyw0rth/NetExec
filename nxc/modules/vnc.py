...
class NXCModule:
    ...
    def vnc_client_proxyconf_extract(self, dploot_conn):
        ...
        for user, sid in users.items():
            ...
            for vnc_name, registry_path, registry_keys in vnc_client_softwares:
                ...
                self.connection.dpapi_triage.log_secret(f"[{vnc_name}] {cred['user']}:{cred['password']}@{cred['server']}", self.context.log)
    ...
    def vnc_from_registry(self, dpapi_conn):
        ...
        for vnc_name, path, key in vncs:
            ...
            if password is None:
                continue
            self.connection.dpapi_triage.log_secret(f"[{vnc_name}] Password: {password.decode('latin-1')}", self.context.log)
