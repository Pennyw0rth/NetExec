...
from errno import EHOSTUNREACH, ENETUNREACH, ETIMEDOUT
...
from impacket.dcerpc.v5.samr import (SAM_MACHINE_ACCOUNT, UF_ACCOUNTDISABLE,
...
from impacket.krb5.kerberosv5 import SessionKeyDecryptionError, getKerberosTGS
...
from nxc.helpers.misc import convert, d2b, get_bloodhound_info
...
class ldap(connection):
        ...
        for message in response:
            ...
            if search_result["resultCode"] == ldapasn1_impacket.ResultCode("success"):
                ...
                if response_value.hasValue():
                    value = response_value.asOctets().decode(response_value.encoding)[
                        2:
                    ]
    ...
    def check_ldaps_cbt(self):
        ...
        try:
            ...
            # Login failed (wrong credentials). test if we get an error with an existing, but wrong CBT -> When supported
                ...
                new_cbv = bytearray(
                    ldap_connection._LDAPConnection__channel_binding_value
                )
    ...
    def enum_host_info(self):
        ...
        self.hostname = (
            self.target.split(".")[0].upper() if "." in self.target else self.target
        )
        ...
        self.output_filename = os.path.expanduser(
            os.path.join(NXC_PATH, "logs", filename)
        )
    ...
    def print_host_info(self):
        ...
        signing = (
            colored("signing:Enforced", host_info_colors[0], attrs=["bold"])
            if self.signing_required
            else colored("signing:None", host_info_colors[1], attrs=["bold"])
        )
        cbt_status = (
            colored(
                f"channel binding:{self.cbt_status}",
                host_info_colors[3],
                attrs=["bold"],
            )
    ...
    def kerberos_login(
        ...
        self.username = (
            username if not self.args.use_kcache else self.username
        )  # With ccache we get the username from the ticket
        ...
        if nthash:
            self.nthash = nthash
        if getattr(self.args, "rusthound", False):
            try:
                self.rusthound()  # collect and optional ingest
            except Exception as e:
                self.logger.fail(f"RustHound-CE failed: {e}")
        ...
        kerb_pass = (
            next(s for s in [self.nthash, password, aesKey] if s)
        ...
        try:
            ...
            if str(e).find("strongerAuthRequired") >= 0:
                ...
                try:
                    ...
                    if not self.args.local_auth and self.username != "":
                        add_user_bh(
                            self.username, self.domain, self.logger, self.config
                        )
                    if self.admin_privs:
                        add_user_bh(
                            f"{self.hostname}$", domain, self.logger, self.config
                        )
                    return True
                except SessionError as e:
                    error, desc = e.getErrorString()
            ...
            if str(e).find("strongerAuthRequired") >= 0:
                ...
                try:
                    ...
                    self.logger.fail(
                        ...
                        color=(
                            "magenta"
                            if (error_code in ldap_error_status and error_code != 1)
                            else "red"
                        ),
                    )
            else:
    ...
    def check_if_admin(self):
        ...
        if (
            ...
            resp = self.search(
                search_filter, attributes, sizeLimit=0, baseDN=self.baseDN
            )
            resp_parsed = parse_result_attributes(resp)
            answers = []
            for item in resp_parsed:
                answers.append(
                    f"(memberOf:1.2.840.113556.1.4.1941:={item['distinguishedName']})"
                )
            if len(answers) == 0:
                self.logger.debug(
    ...
    def search(
        self, searchFilter, attributes, sizeLimit=0, baseDN=None, searchControls=None
    ) -> list:
        ...
        try:
            if self.ldap_connection:
                ...
                paged_search_control = (
                    [
                        ldapasn1_impacket.SimplePagedResultsControl(
                ...
                return self.ldap_connection.search(
                    ...
                    sizeLimit=sizeLimit,
                    searchControls=(
                        searchControls if searchControls else paged_search_control
                    ),
                )
    ...
    def users(self):
        ...
        if self.args.users:
            ...
            search_filter = (
                f"(|{''.join(f'(sAMAccountName={user})' for user in self.args.users)})"
            )
        else:
                ...
                if pwd_last_set:
                    pwd_last_set = (
                        ...
                        if pwd_last_set == "0"
                            ...
                            self.getUnixTime(int(pwd_last_set))
    ...
    def dc_list(self):
        ...
        for trust in trust_resp_parse:
            try:
                ...
                trust_attributes_text = (
                    ", ".join(
                        text
                        for flag, text in trust_attribute_flags.items()
                        if trust_attributes & flag
                    )
                    or "Other"
                )  # If Trust attrs not known
    ...
    def active_users(self):
        ...
        if resp:
            ...
            active_users = [
                ...
                if not (
                    int(user.get("userAccountControl", UF_ACCOUNTDISABLE))
                    & UF_ACCOUNTDISABLE
                )
            ]
            ...
            for user in enabled:
                ...
                if TGT:
                    ...
                    try:
                        ...
                        pwdLastSet = (
                            "<never>"
                            if str(user.get("pwdLastSet", 0)) == "0"
                            else str(
                                datetime.fromtimestamp(
                                    self.getUnixTime(int(user["pwdLastSet"]))
                                )
                            )
                        )
                        lastLogon = (
                            "<never>"
                            if str(user.get("lastLogon", 0)) == "0"
                            else str(
                                datetime.fromtimestamp(
                                    self.getUnixTime(int(user["lastLogon"]))
                                )
                            )
                        )
                        self.logger.display(
                        ...
                        if self.args.kerberoasting:
                            with open(
                                self.args.kerberoasting, "a+"
                            ) as hash_kerberoasting:
    ...
    def find_delegation(self):
        def printTable(items, header):
            ...
            for i, col in enumerate(header):
                rowMaxLen = (
                    max(len(row[1].split(",")[0].split("CN=")[-1]) for row in items)
                    if i == 1
                    else max(len(str(row[i])) for row in items)
                )
        ...
        # Building the search filter
        search_filter = (
            f"(&(|(UserAccountControl:1.2.840.113556.1.4.803:={UF_TRUSTED_TO_AUTHENTICATE_FOR_DELEGATION})"
        ...
        for item in resp_parsed:
            ...
            try:
                ...
                if (
                    ...
                    and not userAccountControl & UF_SERVER_TRUST_ACCOUNT
                ):
                ...
                if item.get("msDS-AllowedToActOnBehalfOfOtherIdentity") is not None:
                    ...
                    if len(sd["Dacl"].aces) > 0:
                        ...
                        for rights, objType in zip(
                            rbcdRights, rbcdObjType, strict=True
                        ):
                            answers.append(
                                [
                                    rights,
    ...
    def password_not_required(self):
        ...
        if resp_parsed:
            for user in resp_parsed:
                status = (
                    "disabled" if int(user["userAccountControl"]) & 2 else "enabled"
                )
                self.logger.highlight(
    ...
    def decipher_gmsa_name(self, domain_name=None, account_name=None):
        ...
        bin_hash = hmac.new(
            bytes("", "latin-1"), msg=bin_account_name, digestmod=hashlib.sha256
        ).digest()
        ...
        if self.args.gmsa_convert_id:
            if len(self.args.gmsa_convert_id) != 64:
                ...
                if gmsa_accounts_parsed:
                    ...
                    for acc in gmsa_accounts_parsed:
                        if (
                            self.decipher_gmsa_name(
                                self.domain.split(".")[0], acc["sAMAccountName"][:-1]
                            )
                            == self.args.gmsa_convert_id
                        ):
                            self.logger.highlight(
    ...
    def pso(self):
        ...
        resp = self.search(
            ...
            baseDN=f"CN=Password Settings Container,CN=System,{self.baseDN}",
            attributes=[],
        )
        if len(resp) > 1:
    ...
    def pass_pol(self):
        ...
        for policy in resp_parsed:
            ...
            lock_accnt_dur = convert(
                0,
                int(lockout_duration_val) if lockout_duration_val != "0" else 0,
                lockout=True,
            )
    ...
    def bloodhound(self):
        ...
        use_bhce = self.config.getboolean(
            "BloodHound-CE", "bhce_enabled", fallback=False
        )
        ...
        collect = resolve_collection_methods(
            "Default" if not self.args.collection else self.args.collection
        )
        if not collect:
            return
        self.logger.highlight(
            "Resolved collection methods: " + ", ".join(list(collect))
        )
        ...
        try:
            ...
            self.logger.fail(
                "Bloodhound-python failed to resolve domain information, try specifying the DNS server."
            )
            return
        ...
        try:
            ...
            self.logger.fail(
                f"BloodHound collection failed: {e.__class__.__name__} - {e}"
            )
            self.logger.debug(
                f"BloodHound collection failed: {e.__class__.__name__} - {e}",
                exc_info=True,
        ...
        self.logger.highlight(
            f"Compressing output into {self.output_filename}bloodhound.zip"
        )
